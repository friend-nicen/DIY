		...
		<view class="uni-easyinput__content" :class="inputContentClass" :style="inputContentStyle">
			...
			<template v-if="type === 'password' && passwordIcon">
				...
				<uni-icons v-if="isVal" class="content-clear-icon" :class="{'is-textarea-icon':type==='textarea'}"
					...
					:color="focusShow?'#07c160':'#c0c4cc'" @click="onEyes">
				</uni-icons>
			</template>
			...
			<template v-else>
				<uni-icons v-if="clearable && isVal && !disabled  && type !== 'textarea'" class="content-clear-icon"
					...
					:color="msg?'#dd524d':(focusShow?'#07c160':'#c0c4cc')" @click="onClear"></uni-icons>
			</template>
...
<script>
	...
	export default {
		...
		computed: {
			...
			inputContentStyle() {
				const focusColor = this.focusShow ? '#07c160' : this.styles.borderColor
